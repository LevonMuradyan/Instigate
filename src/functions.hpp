...
/**
@brief calculating each word how many times is repeated
@param m - map that provides word -> count mapping
*/
void extract_words_count(const std::map<std::string, size_t>& m);

/**
@brief writing generated matrices in to the file
@param matrices_filename - filename of file in which we are writing
@param matrices - vector of matrices 
*/
void write_matrices_in_file(const std::string& matrices_filename, const std::vector<std::bitset<100>>& matrices);
